...
import { useSocket } from "@/hooks/useSocket"; // ✅ Add Socket.io hook
...
interface Message {
  ...
  isOptimistic?: boolean; // ✅ Add optimistic flag
}
...
export default function MessagesPage() {
  ...
  const {
    ...
    startTyping,
    stopTyping,
  }: UseSocketReturn = useSocket({
    // Handle new messages
    onNewMessage: useCallback(
      (socketMessage: SocketMessage) => {
        if (socketMessage.conversationId === selectedConversationId) {
          const newMsg: Message = {
            ...
            canEdit: socketMessage.senderId === session?.user?.id,
            canDelete: socketMessage.senderId === session?.user?.id,
            tempId: socketMessage.tempId,
            isOptimistic: false,
          };
        ...
        setConversations((prev: Conversation[]) =>
          prev.map((conv: Conversation) =>
            conv.id === socketMessage.conversationId
              ? {
                  ...conv,
                  lastMessage: {
                    id: socketMessage.id,
    ...
    onMessageUpdate: useCallback((tempId: string, socketMessage: SocketMessage) => {
      setMessages((prev: Message[]) =>
        prev.map((msg: Message) =>
          msg.tempId === tempId
            ? { ...msg, id: socketMessage.id, isOptimistic: false }
            : msg
        )
      );
    }, []),
  } satisfies UseSocketProps);
      ...
      if (jobId && customerId && jobTitle) {
        ...
        const response = await fetch("/api/conversations", {
          ...
          body: JSON.stringify({
            ...
            initialMessage: `Hi ${handymanName}! I'd like to discuss a ${
              service || "project"
            } and get a quote.`,
          }),
        });
  ...
  // ✅ Enhanced sendMessage with Socket.io + your original REST API
  const sendMessage = async () => {
    ...
    try {
      ...
      let tempId = "";
      if (isConnected) {
        tempId = socketSendMessage(selectedConversationId, messageContent);
      ...
      // Your original REST API call for persistence
      ...
      const data = await response.json();
      if (data.success) {
        // If Socket.io didn't work, add message manually
        if (!isConnected) {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded opacity-0 group-hover:opacity-100 transition-all duration-200"
